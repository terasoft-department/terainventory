...
class DodomaSalesController extends Controller
{
    // Get all sales
    public function index()
    {
        try {
            // Eager load the item relationship and filter where distribution is Dodoma_showRoom
            $sales = Sale::with('item')
                ->whereHas('item', function ($query) {
                    $query->where('distribution', 'Dodoma_showRoom'); // Check the distribution field
                })
                ->get();
    ...
    {
        try {
            // Find the sale by its ID and filter where distribution is Dodoma_showRoom
            $sale = Sale::with('item')
                ->whereHas('item', function ($query) {
                    $query->where('distribution', 'Dodoma_showRoom'); // Check the distribution field
                })
    ...
    public function store(Request $request)
    {
        ...
        try {
            // Find the item by its ID and check if distribution is Dodoma_showRoom
            $item = Item::where('item_id', $validatedData['item_id'])
                ->where('distribution', 'Dodoma_showRoom') // Check the distribution field
                ->firstOrFail();
    ...
    // Get all cash sales
    public function cashSalesDodoma()
    {
        try {
            // Eager load the item relationship and filter by distribution 'Dodoma_showRoom'
            $sales = Sale::with('item')
    ...
    public function creditSalesDodoma()
    {
        try {
            // Eager load the item relationship and filter by distribution 'Dodoma_showRoom'
            $sales = Sale::with('item')
    ...
    // Filter sales report dodoma_showRoom by date
    public function filterByDateDodoma(Request $request)
    {
        ...
        try {
            ...
            $sales = Sale::with('item')
                ->whereHas('item', function ($query) {
                    $query->where('distribution', 'Dodoma_showRoom');
                })
